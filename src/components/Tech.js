...
export default function Tools() {
  ...
  return (
    <div name="tech" className='h-full w-full justify-center bg-background dark:bg-darkBackground text-darkText dark:text-text'>
      <div className='max-w-screen-lg px-8 p-4 mx-auto flex flex-col justify-center h-full w-full'>
        <div className='mt-20'> {/* Navbar offset */}
          ...
          <p className='text-l font-bold text-center md:text-left'>Here are some of my favorite technologies</p>
          ...
          { technologies.map(({id, src, title, style}) =>{
              return(
                <div key={id} className={`py-2 rounded-lg ${style}`}>
                    ...
                    <p className='text-xl font-bold pt-3 text-center'>{title}</p>
                </div>
            )})
          }
          </div>
